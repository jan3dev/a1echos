...
12. **CNG (Continuous Native Generation)** - `ios/` and `android/` directories are generated by `npx expo prebuild` and must never be manually edited. All native modifications must go through Expo config plugins in `plugins/`. These directories may be regenerated at any time.
